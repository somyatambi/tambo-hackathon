      ...
      <div className="messages-area">
        {(!thread?.messages || thread.messages.length === 0) && (
          <div className="empty-state">
            <h3 style={{ marginBottom: '20px', color: '#666' }}>How can I help you manage tasks?</h3>
            {/* ✨ FIXED: Added Suggestion Buttons back */}
            <div className="suggestion-chips">
              <button className="chip" onClick={() => setValue("Show my pending tasks")}>
                Show pending tasks
              </button>
              <button className="chip" onClick={() => setValue("Add a new task")}>
                Add a new task
              </button>
              <button className="chip" onClick={() => setValue("Show all tasks")}>
                Show all tasks
              </button>
            </div>
          </div>
            ...
            <div key={i} className={`message-wrapper ${m.role}`}>
               ...
               {m.component && (
                 <div className="component-wrapper">
                   {m.component.renderedComponent || <TaskList title="My Tasks" />}
                 </div>
               )}
            </div>
          );
