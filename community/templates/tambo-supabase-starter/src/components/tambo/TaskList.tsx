...
export function TaskList({ filter = 'all', title = 'My Tasks' }: TaskListProps) {
  ...
  const addTask = async (e: React.FormEvent) => {
    ...
    // Database Insert
    const { data, error } = await supabase.from('todos').insert([{ text: tempTask.text }]).select();
    ...
    <div className="task-list-container">
      <div className="task-list-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <h3 className="task-list-title">
          {title.toUpperCase()} ({filter === 'pending' ? 'PENDING' : filter === 'completed' ? 'COMPLETED' : 'ALL'})
        </h3>
      </div>

      {/* Input Form for New Task (Added this) */}
      <form onSubmit={addTask} style={{ display: 'flex', gap: '8px', marginBottom: '16px', paddingBottom: '16px', borderBottom: '1px solid #f0f0f0' }}>
        <input 
          value={newTask} 
          onChange={(e) => setNewTask(e.target.value)} 
          placeholder="Add a new task..." 
          style={{ flex: 1, padding: '8px 12px', borderRadius: '8px', border: '1px solid #e5e5e5', outline: 'none', fontSize: '14px' }}
        />
        <button type="submit" style={{ background: '#2563eb', color: 'white', border: 'none', padding: '8px 16px', borderRadius: '8px', cursor: 'pointer', fontSize: '13px', fontWeight: 500 }}>
          Add
        </button>
      </form>
      ...
      {!loading && tasks.length > 0 && (
        <div className="tasks-list">
          {tasks.map(t => (
            <div key={t.id} className={`task-item ${t.is_complete ? 'completed' : ''}`} style={{ position: 'relative' }}>
              ...
              <button 
                ...
                style={{ marginLeft: 'auto', background: 'transparent', border: 'none', color: '#ef4444', cursor: 'pointer', padding: '4px', display: 'flex', alignItems: 'center' }}
                title="Delete task"
              >
